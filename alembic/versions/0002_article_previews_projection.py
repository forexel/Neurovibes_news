...
def upgrade() -> None:
    ...
    op.execute(
        ...
        BEGIN
          ...
          INSERT INTO public.article_previews (
            ...
            NEW.id, lower(NEW.status::text), NEW.content_mode, NEW.double_of_article_id, NEW.title, NEW.subtitle, NEW.ru_title, NEW.ru_summary, NEW.short_hook, NEW.source_id,
    ...
    op.execute(
        ...
        SELECT
          a.id, lower(a.status::text), a.content_mode, a.double_of_article_id, a.title, a.subtitle, a.ru_title, a.ru_summary, a.short_hook, a.source_id,
    ...
    op.execute("UPDATE public.article_previews SET status = lower(status) WHERE status IS NOT NULL")
